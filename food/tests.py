...
class DeleteCommentViewTest(TestCase):
    ...
    # Create a user and log them in
            ...
            self.comment = Comment.objects.create(user=self.user, item=self.item, text='Test comment')

            # URL for delete comment view
            self.url = reverse('food:delete_comment', kwargs={'item_pk': self.item.pk, 'comment_pk': self.comment.pk})

    def test_render_template(self):

        # Send GET request to the delete comment view
        response = self.client.get(self.url)
        print("Response content:", response.content)

        print("Template used:", response.template_name)

        # Check if the correct template is rendered
        self.assertTemplateUsed(response, 'food/comment-delete.html')

        # Check if the item name is displayed in the template
        self.assertContains(response, self.comment.item.item_name)

        # Check if the comment text is displayed in the template
